<!-- version: 1.0.0 -->
<!-- classification: SUMMARY -->
<!-- date: 2026-06-19 -->
<!-- last-updated: 2026-06-19 -->
<!-- status: ACTIVE -->

# changes/ — Change Summaries

Before/after records of code and behavior changes introduced by a feature or fix.

## Summary

Each doc captures what changed (before vs after), behavioral deltas, and performance notes for a
specific change. For ongoing roadmap see [`../WAVE-PLAN.md`](../WAVE-PLAN.md).

## Contents (linear)

1. [`CHANGE-LOGS-PANEL-2026-06-09.md`](CHANGE-LOGS-PANEL-2026-06-09.md) — before/after of the Logs
   panel implementation (Wave 5.5): code changes, behavioral changes, performance.

## Reference

Glossary: [`../documentation/GLOSSARY.md`](../documentation/GLOSSARY.md)
