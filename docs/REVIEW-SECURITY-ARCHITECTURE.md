...
```mermaid
graph TB
  subgraph CLI["CLI Entry (src/index.ts + src/cli.ts)"]
    IDX[index.ts<br/>VERSION, routes args]
    CLI_CMD[cli.ts<br/>commander: doctor subcommand]
  ...
  subgraph APP["App (src/app.ts)"]
    direction TB
    APP[App<br/>terminal lifecycle<br/>tabs / resize]
    TOOLS_REG[registerTools<br/>Bash · Read · Write · WebFetch]
    APP --> TOOLS_REG
  end

  subgraph TUI_INPUT["TUI Input Layer"]
    STDIN[process.stdin raw mode]
    KBD[keyboard.ts<br/>parseKey]
    MOUSE[mouse.ts<br/>parseMouse]
    ROUTER[InputRouter<br/>dispatch]
    STDIN --> KBD
    STDIN --> MOUSE
    KBD --> ROUTER
    MOUSE --> ROUTER
  ...
  subgraph TUI_RENDER["TUI Render Layer"]
    ANSI[ansi.ts<br/>ANSI escape builders]
    BUF[CellBuffer<br/>cell grid + diff]
    LAYOUT[layout.ts<br/>Rect computation]
    THEME[theme.ts<br/>256-color palette]
    BUF --> ANSI
    LAYOUT --> BUF
    THEME --> BUF
  end
  ...
  subgraph PANELS["Panels"]
    SESS[SessionPanel<br/>chat + input bar]
    CANVAS[OrchestrationCanvas<br/>drag-drop blocks]
    AGENTS[AgentsPanel<br/>status sidebar]
    STATUSBAR[StatusBar<br/>bottom line]
  end

  subgraph WIDGETS["Widgets"]
    BLOCK_W[Block.ts<br/>double-line box]
    WIRE_W[Wire.ts<br/>L-shape routing]
    CTX_MENU[ContextMenu.ts<br/>right-click popup]
    CMD_PAL[CommandPalette.ts<br/>🔲 planned]
  end

  subgraph CORE["Core Engine"]
    LOOP[agentLoop<br/>async generator]
    SESSION[Session<br/>history + tokens]
    HOOKS[hooks.ts<br/>shell runners]
    DISPATCH[tools/index.ts<br/>Zod registry + dispatch]
  end

  subgraph TOOLS["Built-in Tools"]
    BASH[BashTool<br/>exec 🔴]
    READ[ReadTool<br/>readFile 🔴]
    WRITE[WriteTool<br/>writeFile 🔴]
    FETCH[WebFetchTool<br/>fetch 🔴]
  end

  subgraph HARNESS["Harness"]
    DOCTOR[doctor.ts<br/>env checks]
    READER[reader.ts 🔲]
...
sequenceDiagram
  ...
  UI->>LOOP: for await (agentLoop(session))
  Note over LOOP: turns = 0; maxTurns = 20

  loop Each turn (while turns < maxTurns)
    LOOP->>HOOKS: runHook('StepStart', {turn})
    LOOP->>API: messages.create({stream:true})

    loop Streaming events
      API-->>LOOP: content_block_start (tool_use)
      Note over LOOP: register toolBlock[index] = {id, name, inputAccum: ''}
      LOOP-->>UI: AgentEvent {type:'tool_start'}

      API-->>LOOP: content_block_delta (text_delta)
      LOOP-->>UI: AgentEvent {type:'text_delta', delta}
      Note over UI: currentLine.text += delta → onUpdate()

      API-->>LOOP: content_block_delta (input_json_delta)
      Note over LOOP: toolBlock.inputAccum += partial_json ← never parse here

      API-->>LOOP: message_delta (stop_reason)
      Note over LOOP: state.stopReason = event.delta.stop_reason
    end

    Note over LOOP: Stream ends. Now dispatch all tool calls.

    loop For each toolBlock
      Note over LOOP: JSON.parse(block.inputAccum) ← only here
      LOOP->>HOOKS: runHook('PreToolUse', {tool, input})
      alt hook returns continue=false
        Note over LOOP: result = "blocked by PreToolUse hook"
      else
        LOOP->>TOOLS: dispatch(name, parsedInput)
        TOOLS-->>LOOP: result: string
        LOOP->>HOOKS: runHook('PostToolUse', {tool, result})
      end
      LOOP-->>UI: AgentEvent {type:'tool_result'}
    end

    LOOP->>HOOKS: runHook('StepComplete', {turn, stop_reason})
    LOOP-->>UI: AgentEvent {type:'turn_end'}

    alt stop_reason === 'end_turn' OR no tool calls
      Note over LOOP: break — done
    else tool_use stop
      Note over LOOP: add tool_result message, turns++, continue
    end
  end
...
```mermaid
flowchart TD
  A[App.render called\nvia onUpdate callback] --> B[computeLayout\nrows/cols → PanelLayout]
  B --> C[buf.fill entire screen\nwith bg color]
  C --> D[renderTabBar\nwrite tab labels to row 0]
  D --> E{Active tab}
  ...
  subgraph CellBuffer diff algorithm
...
```mermaid
flowchart TD
  STDIN[process.stdin 'data' event\nBuffer] --> TRY_MOUSE{parseMouse\nstarts with ESC [ <}

  TRY_MOUSE -->|null| PARSE_KEY[parseKey\nraw buffer → KeyEvent]
  ...
  GLOBAL_KEYS -->|Tab| TAB_CYCLE[activeTab = next % 4]
  GLOBAL_KEYS -->|1-4| TAB_DIRECT[activeTab = key-1]
  GLOBAL_KEYS -->|other| KB_ROUTE[router.dispatch key\npanels, activeTab]

  KB_ROUTE --> FOCUSED[panels.at activeTab\n→ panel.onKey e]

  MOUSE_ROUTE[router.dispatch mouse\npanels, activeTab] --> HIT_TEST{for each panel\nhit-rect test}
  HIT_TEST -->|match| PANEL_MOUSE[panel.onMouse e]
  HIT_TEST -->|no match| MOUSE_DROP[return false]

  PANEL_MOUSE --> CANVAS_LOGIC{OrchestrationCanvas\nonMouse}
  CANVAS_LOGIC -->|right press| CONTEXT_MENU[openContextMenu]
  CANVAS_LOGIC -->|left press| HIT_HEADER[hitTestHeader\n→ start drag]
  CANVAS_LOGIC -->|left move| GHOST[update ghostRow/ghostCol]
  CANVAS_LOGIC -->|left release| SNAP[snap to grid\nupdate block pos]
```
...
stateDiagram-v2
  ...
  note right of dragging
    ghostRow = row - offsetRow
    ghostCol = col - offsetCol
    renders: real block at original pos
             faded ghost at cursor pos
  end note

  dragging --> idle : mouse release (left)
  note right of idle
    snappedRow = round(rawRow / GRID_ROWS) * GRID_ROWS
    snappedCol = round(rawCol / GRID_COLS) * GRID_COLS
  ...
  idle --> idle : right press → context menu
  idle --> idle : left press miss (no block)

  dragging --> dragging : right press (ignored\nwhile dragging)
```
...
```mermaid
flowchart TD
  LOOP[agentLoop\n— tool calls collected] --> PARSE[JSON.parse inputAccum\nper tool block]
  PARSE --> HOOK_PRE[runHook PreToolUse\n.ai/hooks/PreToolUse.sh]

  HOOK_PRE -->|exit 0| ZOD[tool.inputSchema\n.safeParse rawInput]
  HOOK_PRE -->|exit ≠ 0| BLOCKED["result = 'blocked'"]
  ...
  RESULT --> HOOK_POST[runHook PostToolUse\n.ai/hooks/PostToolUse.sh]
  HOOK_POST --> YIELD[yield tool_result event]
  YIELD --> HISTORY[session.addMessage\ntool_result parts]
